...
class VerdaccioRotatingFileStream extends Logger.RotatingFileStream {
  ...
  write(obj) {
    ...
    super.write(JSON.stringify(_objectSpread({}, obj, {
      msg
    }), Logger.safeCycles()) + '\n');
  }
...
function setup(logs) {
  ...
  logs.forEach(function (target) {
    ...
    if (target.type === 'rotating-file') {
      ...
      const stream = new VerdaccioRotatingFileStream(_.merge({}, // Defaults can be found here: https://github.com/trentm/node-bunyan#stream-type-rotating-file
      target.options || {}, {
        path: target.path,
        level
      }));
      ...
      if (target.format === 'pretty') {
        ...
        stream.write = obj => {
          ...
          destination.write(`${JSON.stringify(_objectSpread({}, obj, {
            msg
          }), Logger.safeCycles())}\n`);
        };
      }
...
function fillInMsgTemplate(msg, obj, colors) {
  return msg.replace(/@{(!?[$A-Za-z_][$0-9A-Za-z\._]*)}/g, (_, name) => {
    ...
    for (let _i = 0; _i < _ref.length; _i++) {
      ...
      if ((0, _utils.isObject)(str) || Array.isArray(str)) {
        str = str[id];
      } else {
        str = undefined;
      }
