...
function loadPlugin(config, pluginConfigs = {}, params, sanityCheck) {
  return Object.keys(pluginConfigs).map(pluginId => {
    let plugin; // try local plugins first

    plugin = tryLoad(_path.default.resolve(__dirname + '/../plugins', pluginId)); // try the external plugin directory

    if (plugin === null && config.plugins) {
      const pluginDir = config.plugins;
      plugin = tryLoad(_path.default.resolve(pluginDir, pluginId)); // npm package

      if (plugin === null && pluginId.match(/^[^\.\/]/)) {
        plugin = tryLoad(_path.default.resolve(pluginDir, `verdaccio-${pluginId}`)); // compatibility for old sinopia plugins
    ...
    if (plugin === null && pluginId.match(/^[^\.\/]/)) {
      plugin = tryLoad(`verdaccio-${pluginId}`); // compatibility for old sinopia plugins
